...
public:
    ...
    //from physic core
    Point2f Normalized();
	Point2f projectOnVector(Point2f vec);
	Point2f getLeftNormal();
	Point2f getRightNormal();
	double ratio(Point2f vec);
	Point2f reflectFrom(Point2f mirror);
    bool isCollinearTo(Point2f vec);
