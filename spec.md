...
Иными словами, менеджен ресурсов, класс, который читает xml, находящийся выше, инициализирует ресурсы и хранит в себе. Примерный интерфейс такого класса:
```C
...
private:
    bool load_image_feature(xml::XMLElement *layer_ptr);
    bool load_sprite_feature(xml::XMLElement *sprite_ptr);
